...
def _parity(permutation: list[int]) -> int:
    """0 for an even permutation, 1 for odd, from how many pairs are out of order."""
    return sum(a > b for a, b in combinations(permutation, 2)) % 2
...
def _pieces(state: str, slots: list[tuple[int, ...]], home: dict, kind: str) -> tuple[list[int], list[str]]:
    ...
    for slot in slots:
        ...
        if piece is None:
            problems.append(f"one {kind} reads {colours}, which isn't a real piece")
